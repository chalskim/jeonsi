...
type ServiceType = '긴급 의뢰 등록' | '추천 전문가 등록' | '추천 교육 등록' | '베너 광고 등록'
...
export default function ServiceHistoryPersonal() {
  ...
  const data = useMemo<{ all: ServiceItem[]; ongoing: ServiceItem[]; completed: ServiceItem[]; cancelled: ServiceItem[] }>(() => ({
    all: [
      {
        id: 1,
        serviceType: '긴급 의뢰 등록',
        serviceName: '웹 애플리케이션 보안 취약점 진단',
        company: '㈜테크솔루션',
      ...
      {
        id: 2,
        serviceType: '추천 전문가 등록',
        serviceName: 'AI/머신러닝 분야 전문가 추천',
        company: '자신의 프로필',
      ...
      },
      {
        ...
        serviceName: '클라우드 아키텍처 자격증 취득 과정',
        company: '교육 플랫폼',
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}> 
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>의뢰 등록, 추천 전문가, 교육 추천 등 이용 내역을 확인하세요</Text>
        </View>
              ...
              {items.map((it) => (
                <TouchableOpacity key={it.id} style={styles.card} activeOpacity={0.8}>
                  <View style={styles.cardHeader}>
                    <Text style={styles.company}>{it.company}</Text>
